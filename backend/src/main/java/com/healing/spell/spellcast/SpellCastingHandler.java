...
public class SpellCastingHandler {
    ...
    public SpellCastingHandler(ActionsQueue actionsQueue, RaiderHandler raiderHandler, GlobalCooldownHandler globalCooldownHandler) {
    ...
    public void startCastingSpell(Spell spell, Player player, Entity target) {
        ...
        spell.startCooldown();
        System.out.println("Started casting " + spell.getName() + " time remaining: " + castTimeRemaining);
    ...
    public void cancelSpellCast() {
        if (isCasting && castingSpell != null) {
            System.out.println("Cancelled casting " + castingSpell.getName() + " with " + castTimeRemaining + " seconds remaining");
            resetSpellCastState();
        }
    }
    ...
    private void finishSpellCast() {
        castingSpell.createAction(
                actionsQueue, target, getAdditionalTargets(target, castingSpell.getAdditionalTargets()), player);
